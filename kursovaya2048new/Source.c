...
int PlayAct(PDISPLAY GameBoard);  // Обновляем поле
int RecallBoard(PDISPLAY GameBoard, int Command);   // Вернуться на один ход
void Game();
...
int WinOrLose(PDISPLAY GameBoard)
{
    ...
    if (GameBoard->count == SIZE_BOARD * SIZE_BOARD && IsRound(GameBoard, 0, 0) == 0) return -1;
    return 0;
}
...
void SetBlock(PDISPLAY GameBoard, int flag, int uplimit, int i, int j)
{
    ...
    for (k = 0; k < uplimit; k++)
    {
        ...
        else if (GameBoard->board[i][j] == GameBoard->board[i + pozXY[flag][k].x][j + pozXY[flag][k].y] && GameBoard->flag[i + pozXY[flag][k].x][j + pozXY[flag][k].y] == 0)
        {
        ...
        else if (GameBoard->board[i][j] != GameBoard->board[i + pozXY[flag][k].x][j + pozXY[flag][k].y] || GameBoard->flag[i + pozXY[flag][k].x][j + pozXY[flag][k].y] == 1)
        {
            if (k == 0)
            {
                break;
...
int RecallBoard(PDISPLAY GameBoard, int Command)
{
    int i = 0;
    int j = 0;
